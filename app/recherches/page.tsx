    ...
    <div className="page-shell listing-page">
      ...
      {result.items.length ? <><section className="wishlist-list" aria-label="Objets recherchés">{result.items.map((item) => {
        ...
        return <article className={`wishlist-row priority-${item.priority}`} key={item.id}>
          ...
          <div><p className="reference">{variant.references[0]?.displayValue ?? variant.product.baseReference ?? variant.canonicalKey}</p><h2><Link href={`/sets/${variant.id}`}>{name}</Link></h2><p className="priority-label">Priorité : <strong>{priorityLabel[item.priority] ?? "Normale"}</strong></p>{item.notes && <p className="item-notes">{item.notes}</p>}</div>
      ...
      })}</section>{result.pages > 1 && <nav className="pagination simple-pagination" aria-label="Pagination des recherches">{page > 1 ? <Link href={pageHref(page - 1)}>← Précédente</Link> : <span aria-disabled="true">← Précédente</span>}<span>Page {page} sur {result.pages}</span>{page < result.pages ? <Link href={pageHref(page + 1)}>Suivante →</Link> : <span aria-disabled="true">Suivante →</span>}</nav>}</> : <section className="empty-state collector-empty wanted-empty"><span aria-hidden="true">♡</span><h2>Aucune recherche pour le moment</h2><p>Ajoutez les boîtes qui vous intéressent depuis le catalogue.</p><Link className="button" href="/catalogue">Explorer le catalogue</Link></section>}
    </div>
  );
}
